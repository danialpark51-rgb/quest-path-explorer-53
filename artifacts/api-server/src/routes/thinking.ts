...
// Call the available AI service — tries Gemini first, falls back to OpenAI
...
- If the name is misspelled but recognizable (e.g. "Einstien" → Einstein), analyze the correct person and note the correction.
...
Write with warmth, depth, and precision. This is for students who want to model their thinking after the world's greatest minds.`;
  ...
  if (!raw) {
    res.status(503).json({ error: "AI service is not configured. Please add an AI API key to enable this feature." });
    return;
  }
